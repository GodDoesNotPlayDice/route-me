...
export class DriverCarMother{
	static random() :Result<DriverCar, Error[]>{
		return Ok({
			id: DriverCarIDMother.random().unwrap(),
			model: DriverCarModelMother.random().unwrap(),
			seat: DriverCarSeatMother.random().unwrap(),
		})
	}

	static invalid() :Result<DriverCar, Error[]>{
		return Err([
			DriverCarIDMother.invalid().unwrapErr(),
			DriverCarModelMother.invalid().unwrapErr(),
			DriverCarSeatMother.invalid().unwrapErr(),
		])
	}
}
