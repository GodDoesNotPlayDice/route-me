...
export class CurrencyDaoCurrencyExchange implements CurrencyDao{
	...
	async getCurrencyExchange( from: string, to: string): Promise<Result<CurrencyExchange, Error>> {
		...
		if ( response === undefined ) {
			return Err(new UnknownException( 'currency exchange dao. get currency exchange. currency exchange' ) )
		}
